...
def attack(model, x, y, theta=1., clip_min=0., clip_max=1., max_iter=60, cast=tf.float32):
    """
    Tensorflow implementation of the Jacobian-based Saliency Map Method (Papernot et al. 2016) for targeted attacks.
    Paper link: https://arxiv.org/pdf/1511.07528.pdf
    Based on the original implementation from cleverhans
    See https://github.com/tensorflow/cleverhans/blob/master/cleverhans/attacks/saliency_map_method.py
    ...
    def body(x_in, y_in, domain_in, i_in, cond_in):
        ...
        grads_other = tf.reduce_sum(derivatives * other_classes, axis=0)
        ...
        cond = tf.equal(tf.reduce_sum(y_in * preds_onehot, axis=1), 0) & (tf.reduce_sum(domain_in, axis=1) >= 2)
