...
if __name__ == "__main__":
    ...
    if attack in NON_TARGETED_ATTACKS:
        from attacks import generate_attacks_non_targeted_substitute
        ...
        generate_attacks_non_targeted_substitute(
            attack=attack_non_target, model_path=model, oracle_path=oracle, save_path=save_path, x_set=x, y_set=y,
            first_index=first_index, last_index=last_index, batch_size=batch_size, theta=theta, clip_min=clip_min,
            clip_max=clip_max,  max_iter=max_iter, use_logits=use_logits, non_stop=non_stop
        )
        ...
        generate_attacks_non_targeted_substitute(
            attack=attack_non_target, model_path=model, oracle_path=oracle, save_path=save_path, x_set=x, y_set=y,
            first_index=first_index, last_index=last_index, batch_size=batch_size, theta=None, clip_min=clip_min,
            clip_max=clip_max, max_iter=max_iter, use_logits=use_logits, non_stop=non_stop
        )

    else:
        raise ValueError(f"Unsupported / unknown attack {attack}")
